...
2. **Onboarding guiado** (`/onboarding` → `OnboardingWizard`, `src/components/onboarding/onboarding-wizard.tsx`) — wizard client-side de 4 passos com estado interno (sem sub-rotas):
   ...
   - **Passo 2 — Recebimento**: cria linhas em `payment_methods` (tipos `pix`/`paypal`/`wise`, versão reduzida do catálogo completo — demais tipos ficam para a aba Pagamentos em Configurações) — pulável via "Pular por agora".
...
- Tela inicial oferece até 6 opções (as financeiras só aparecem se o missionário tem métodos de pagamento configurados) e mostra o link "← Voltar ao perfil de {missionaryName}" (recebe `username` como prop). Ao escolher uma opção, esse link some e dá lugar ao "← Voltar" interno de cada sub-tela (volta pro `choice=null`) — só um link de voltar por vez. Antes `page.tsx` renderizava o link pro perfil fixo por cima do wizard, duplicando com o "← Voltar" interno assim que uma sub-tela era aberta.
...
**Nota de design (não implementada nesta rodada): seguir o missionário vs. seguir um projeto.** `partners` continua sendo o único relacionamento "de conta" (CRM/mensagens/visibilidade granular) — não existe (e não deveria existir) um "parceiro por projeto" separado. Se no futuro fizer sentido notificar só sobre um projeto específico (em vez de tudo que o missionário publica), a peça que falta não é duplicar `partners`, é (a) uma tabela leve tipo `highlight_followers (highlight_id, partner_id)` pra granularidade de notificação, e (b) — mais importante — algum produtor de notificação de verdade, que hoje não existe (ver nota da seção 7.10). `pledges`/`recurring_pledges` já carregam `highlight_id` quando a contribuição nasceu de um projeto específico, o que já serve como sinal de afinidade pronto pra esse momento futuro.
...
| Componente | Papel |
|---|---|
| `sidebar.tsx` | `DashboardSidebar` (nav desktop, 9 itens), `MobileHeader`, `MobileBottomNav` (tab bar mobile; 5º slot é uma miniatura de avatar linkando pra `/[username]`, não mais um botão "Menu" — ver 10 `account-menu-drawer.tsx` e changelog) |
...
| `edit-profile-dialog.tsx` | `EditProfileDialog` — abre o `ProfileForm` (de `dashboard/settings/`) dentro de um `Dialog` por cima do perfil público, sem duplicar a lógica de edição |
| `profile-tabs.tsx` | `ProfileTabs` — barra de abas persistente (Perfil/História/Projetos/Trajetória) renderizada pelo `[username]/layout.tsx`; oculta em `mensagens`/`oracao`/`parceria`/`projetos/[slug]` (ver 7.12). Na variante com abas, quando `canEdit`, monta o trigger de `AccountMenuDrawer` (`dashboard/`) entre as abas e o `LanguageSwitcher` — é o único lugar de onde a gaveta de navegação completa é acionada no mobile |

...
Substituiu o antigo `PostEditor` (formulário único num dialog) por um wizard em tela cheia inspirado no fluxo do Instagram, mas não-cópia (benchmark, não clone). `PostComposerModal` orquestra 4 passos via `usePostComposer` (`use-post-composer.ts`), reaproveitando a mesma API pública `useComposer().openComposer/closeComposer` de antes — nenhum call-site (`new-post-triggers.tsx`, `posts-list.tsx`) mudou.
...
| Componente | Papel |
|---|---|
| `post-card.tsx` | `PostCard` — card de post unificado (substituiu `feed-post-card.tsx` e `publications-feed.tsx` em 2026-07-25, ver 7.9/Changelog): mídia em destaque na proporção `media_aspect_ratio`, bolinhas de carrossel, pinos de marcação sob demanda (`post_tags`), legenda truncada (`line-clamp-2` + "...mais"), curtir/comentar/compartilhar. Usado tanto por `dashboard/feed/feed-list.tsx` quanto por `[username]/page.tsx` — mesma tela para missionário e parceiro |
...
`html, body` têm `overflow-x: hidden` + `overscroll-behavior-x: none` (`src/app/globals.css`) — a página nunca desliza/balança lateralmente ao arrastar o dedo, só rolagem vertical. Não usar `touch-action: pan-y` no `html`/`body` pra isso: por spec, `touch-action` de um ancestral restringe também os descendentes (interseção na cadeia), então bloquearia até áreas de scroll horizontal intencionais (carrossel de mídia do post, etc.). `overscroll-behavior` não tem esse problema — é escopado ao próprio container de scroll, não herda pros filhos. Qualquer componente com scroll horizontal de propósito (carrossel, tabs) deve gerenciar seu próprio `overscroll-behavior`/`touch-action` local, sem depender de nada no nível global.

### Editor de recorte de mídia (`src/components/shared/media-editor/`)
`ImageCropEditor` (post, projeto, capa de perfil) — preview em tela = resultado salvo (WYSIWYG): calcula a mesma proporção natural + cor média que `bakeImage()` usa, então se a foto não bate com a proporção alvo e o zoom está em 1, o preview já mostra "contida" (sobra preenchida com a cor média), igual ao arquivo final — nunca aparenta uma coisa na edição e salva outra. Nesse modo o arrastar fica desativado (não teria efeito no resultado); ao dar zoom (>1) volta a recortar/arrastar normalmente. Zoom: botões +/- (mouse/desktop) + pinça de dois dedos (touch) — sem slider fino, difícil de acertar no celular. O listener de `touchmove` no `window` só chama `preventDefault()` quando o usuário está de fato arrastando/pinçando dentro do editor; fora disso o toque tem que passar livre pra rolagem da página (senão a tela trava e o usuário não consegue chegar no botão de salvar mais abaixo).

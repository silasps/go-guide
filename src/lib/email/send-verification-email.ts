...
export async function sendVerificationEmail(to: string, toName: string, verifyUrl: string, locale?: string | null): Promise<boolean> {
  const appName = process.env.NEXT_PUBLIC_APP_NAME ?? 'go→guide'
  const appUrl = process.env.NEXT_PUBLIC_APP_URL
  const resolvedLocale = isLocale(locale) ? locale : 'pt'
  const t = await getTranslations({ locale: resolvedLocale, namespace: 'VerificationEmail' })
...
<body style="margin:0;padding:0;background:#f4f4f5;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">
      ...
      <table width="520" cellpadding="0" cellspacing="0" style="max-width:520px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
        <tr>
          <td style="background:${BRAND_COLOR};padding:32px 40px;text-align:center;">
            ...
            <h1 style="margin:0;font-size:22px;font-weight:800;color:#ffffff;">${t('emailTitle')}</h1>
          </td>
        </tr>
        <tr>
          <td style="padding:36px 40px;">
            <p style="margin:0 0 16px;font-size:15px;color:#374151;line-height:1.6;">
              ${t('emailGreeting', { comma: toName ? `, ${toName}!` : '!', appName })}
            </p>
  ...
  return sendEmail({ to, toName, subject: t('emailSubject'), html })
}

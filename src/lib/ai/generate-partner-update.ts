...
const MODEL_GENERATE = 'claude-haiku-4-5'
...
export async function generatePartnerUpdate({
  ...
  if (projects.length) contextParts.push(`Projetos em andamento que precisam de apoio (cite pelo menos os que ainda não bateram a meta, com um chamado claro e específico pra ação):\n${describeProjects(projects)}`)

  const response = await client.messages.create({
    model: MODEL_GENERATE,
    max_tokens: 1024,
    system: `Você ajuda um missionário a escrever uma atualização por e-mail pra sua rede de parceiros (apoiadores financeiros e de oração). O texto precisa soar pessoal e caloroso, como se o próprio missionário estivesse contando as novidades a um amigo — nunca como um relatório financeiro ou um comunicado corporativo. Teça os números com naturalidade dentro de frases, nunca como uma lista de dados isolada. Não use markdown, títulos nem bullet points, exceto se for genuinamente natural falar de mais de um projeto separadamente. Parágrafos curtos, separados por linha em branco. Termine com um convite claro e específico à ação quando houver projeto que ainda precisa de apoio. Responda apenas com o JSON pedido.`,
    output_config: {
      format: {
        type: 'json_schema',
        schema: {

...
const EMPTY: PostEngagement = { like_count: 0, comment_count: 0, viewer_has_liked: false, tags: [] }

/** Busca curtidas/comentários/marcações de um lote de posts numa única
 *  rodada de queries e devolve um mapa `postId -> PostEngagement`, para
 *  quem monta a lista mesclar em cada post sem esperar per-row. */
...
export async function enrichWithEngagement(supabase: SupabaseClient<any>, postIds: string[], viewerUserId?: string | null): Promise<Map<string, PostEngagement>> {
  ...
  const [{ data: likes }, { data: comments }, { data: tags }, viewerProfile] = await Promise.all([

  ...
  const [{ data: projects }, { data: posts }] = publicIds.length > 0
    ? await Promise.all([
        supabase.from('highlights')
          .select('id, slug, title, cover_url, cover_position, goal_amount, current_amount, currency, original_locale, title_translations, profile:profiles(id, username, display_name, avatar_url)')
      ...
      <main className="flex-1 max-w-2xl mx-auto w-full px-4 py-8 space-y-8">
        ...
        {projects && projects.length > 0 && (
          <section className="space-y-3">
            <h2 className="font-semibold">Projetos que precisam de apoio</h2>
